...
pt_points_to_update <- dbGetQuery(con,"SELECT lat, lon, foi_lat, foi_lon, foi_description_detailed FROM pt_distance")
...
if (dbExistsTable(con, "pt_distance_encoded")) {
  ...
  dbExecute(con,"
  DELETE FROM pt_distance_encoded
  WHERE distance IS NULL OR duration IS NULL;")
  
  already_coded <- dbGetQuery(con,"SELECT * FROM pt_distance_encoded")
  pt_points_to_update <- pt_points_to_update %>% anti_join(already_coded, by = c("lat",
                                                                                "lon",
                                                                                "foi_description_detailed",
                                                                                "foi_lat",
                                                                                "foi_lon"))
}
...
values <- seq(1,nrow(pt_points_to_update),length.out = chunks)
...
#This process had ~4000 empty values but they were all very far from any PT. 
